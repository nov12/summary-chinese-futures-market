...
class TqdataClient:
    ...
    def login(self) -> bool:
        ...
        try:
            self.api = TqApi(auth=TqAuth(self._username, self._password))
    ...
    def query_contracts(self):
        ...
        # 获得全部合约
        symbols = list(self.api.query_quotes(ins_class="FUTURE", expired=False))
    ...
    def query_history(
        self, contract: str, interval: int = 86400, length: int = 300
    ) -> pd.DataFrame:
        """
    ...
    def generate_extreme_dataframe(self, days: Optional[list] = None) -> pd.DataFrame:
        ...
        df_all.columns = df_all.columns.str.replace("ration", "距离%").str.replace(
            "extrema_day", "天数"
        )
    ...
    def get_dominant(self, symbol) -> str:
        ...
        try:
            dominant_symbol = self.api.query_cont_quotes(product_id=underlying_symbol)[0]
        ...
        if len(dominant_symbol) < 1:
            print("Tqdata Debug 01:", dominant_symbol, re.sub(r"\D", "", dominant_symbol))
            return None
